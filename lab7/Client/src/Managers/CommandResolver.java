...
public class CommandResolver {
     ...
     */
    public static LinkedList<Request> executeScript(String fileName, String username, String hashPassword) {
                ...
                if ("add".equals(command) || "update".equals(command)) {
                    if (lineIterator.hasNext()) {
                        Movie movie = new Movie(
                                ...
                                CollectionClasses.MpaaRating.valueOf(lineIterator.next()),
                                new Person(
                                        lineIterator.next(),
                                        lineIterator.next(),
                        ...
                        requests.add(new Request(command, movie, argument, username, hashPassword));
                    }
                } else if ("execute_script".equals(command)) {
                    if (!scriptsInProgress.contains(absolutePath)) {
                        requests.addAll(executeScript(argument, username, hashPassword));
                    }
                } else {
                    requests.add(new Request(command, null, argument, username, hashPassword));
                }
            }
        } catch (IOException e) {
    ...
    public static Request resolve(String[] args, String username, String hashPassword) throws IncorrectValueException {
        ...
        if (specialCommands.contains(command)) {
            movie = Ask.newMovie(new Console(), username);
        }
        return new Request(command, movie, argument, username, hashPassword);
    }
    public static Request connect(){
        return new Request("connect", null, null, null, null);
    }
}
