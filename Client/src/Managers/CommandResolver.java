...
public class CommandResolver {
    ...
    public static Request resolve(String[] args) throws IncorrectValueException {
        if (args.length == 0 || args.length > 2) {
            throw new IllegalArgumentException("Некорректный запрос. Ожидается 1 или 2 аргумента.");
        }

        String command = args[0].trim();
        if (!commandsWithArgument.contains(command) && !commandsWithoutArgument.contains(command)) {
            throw new IllegalArgumentException("Несуществующая команда: " + command);
        }

        if (args.length == 2 && !commandsWithArgument.contains(command)) {
            throw new IllegalArgumentException("Команда не принимает аргументы: " + command);
        }

        if (args.length == 1 && commandsWithArgument.contains(command)) {
            throw new IllegalArgumentException("Для команды требуется аргумент: " + command);
        }
